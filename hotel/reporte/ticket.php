...
$operacion = ProcesoData::getById($_GET['id']);
	...
	if('1' == '1')
	{
		...
		$pdf->MultiCell(68, 4.2, 'C.I.: '.$operacion->getCliente()->documento, 0,'L',0 ,1);
		...
		$pdf->Cell(8,4,($operacion->precio*$operacion->cant_noche),0,0,'L',1);
		$pdf->Ln(4.5);
		...
		//$final= $total+($operacion->total);
		$final= $total+(($operacion->precio*$operacion->cant_noche)+$operacion->carga_salida_tarde);
		

		$pdf->Text(2, $get_Y+1, '-----------------------------------------------------------------------');
		$pdf->SetFont('Arial','B',8.5);
		
		$pdf->Text(4,$get_Y + 10,'CARGA SALIDA TARDE :');
		$pdf->Text(57,$get_Y + 10,number_format($operacion->carga_salida_tarde,2,'.',','));
		$pdf->Text(4,$get_Y + 15,'SUBTOTAL :');
		$pdf->Text(57,$get_Y + 15,number_format(($final/(($iva/100)+1)),2,'.',','));
		$pdf->Text(4,$get_Y + 20,'TOTAL IVA :');
		$pdf->Text(57,$get_Y + 20,number_format($final-($final/(($iva/100)+1)),2,'.',','));
		$pdf->Text(4,$get_Y + 25,'TOTAL A PAGAR :');
		$pdf->Text(57,$get_Y + 25,number_format($final,2,'.',','));
