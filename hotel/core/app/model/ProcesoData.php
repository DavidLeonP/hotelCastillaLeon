...
class ProcesoData {
	...
	public function addIngreso(){
		$sql = "insert into proceso (id_habitacion,id_tarifa,id_cliente,precio,cant_noche,dinero_dejado,id_tipo_pago,fecha_entrada,fecha_salida,total,id_usuario,cant_personas,id_caja,fecha_creada,cantidad,pagado) ";
		$sql .= "value ($this->id_habitacion,$this->id_tarifa,\"$this->id_cliente\",\"$this->precio\",\"$this->cant_noche\",\"$this->dinero_dejado\",1,\"$this->fecha_entrada\",\"$this->fecha_salida\",\"$this->total\",$this->id_usuario,\"$this->cant_personas\",\"$this->id_caja\",$this->fecha_creada,$this->cantidad,$this->pagado)";
...
// partiendo de que ya tenemos creado un objecto PersonaData previamente utilizamos el contexto
	...
	/*public function updateSalida(){
		$sql = "update ".self::$tablename." set carga_salida_tarde=\"$this->carga_salida_tarde\", total=\"$this->total\",id_caja=$this->id_caja,id_tipo_pago=$this->id_tipo_pago,estado=1, pagado=1 where id=$this->id";
		Executor::doit($sql);
	}*/
	/*public function updateSalida(){
		$sql = "update ".self::$tablename." set carga_salida_tarde=\"$this->carga_salida_tarde\",cant_noche=cant_noche+\"$this->dias_estadia\",total=\"$this->total\",id_caja=$this->id_caja,id_tipo_pago=$this->id_tipo_pago,estado=1, pagado=1 where id=$this->id";
		Executor::doit($sql);
	}*/
	public function updateSalida(){
		$sql = "update ".self::$tablename." set carga_salida_tarde=\"$this->carga_salida_tarde\",total=\"$this->total\",id_caja=$this->id_caja,id_tipo_pago=$this->id_tipo_pago,estado=1, pagado=1,fecha_salida=\"$this->fecha_salida\"  where id=$this->id";
		Executor::doit($sql);
	}
